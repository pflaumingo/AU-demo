class SFPB < ZoneHVACEquipment
  ...
  def add_air_terminal
    if self.air_terminal_type == 'Reheat'
      sfpb = OpenStudio::Model::AirTerminalSingleDuctSeriesPIUReheat.new(self.model, self.supply_fan, self.heating_coil)
    else
      sfpb = OpenStudio::Model::AirTerminalSingleDuctSeriesPIUReheat.new(self.model, self.supply_fan, self.heating_coil)
    end

    sfpb.setName(self.name) unless self.name.nil?
    sfpb.additionalProperties.setFeature('id', self.id) unless self.id.nil?
    sfpb.additionalProperties.setFeature('CADObjectId', self.cad_object_id) unless self.cad_object_id.nil?
    sfpb
  end
