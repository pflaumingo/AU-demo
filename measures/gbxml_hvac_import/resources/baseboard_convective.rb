class BaseboardConvective < HVACObject
  ...
  def add_baseboard_convective
    ...
    if self.heating_coil_type == 'ElectricResistance'
      baseboard = OpenStudio::Model::ZoneHVACBaseboardConvectiveElectric.new(self.model)
    elsif self.heating_coil_type == 'HotWater'
      baseboard = OpenStudio::Model::ZoneHVACBaseboardConvectiveWater.new(self.model, self.model.alwaysOnDiscreteSchedule, self.heating_coil)
    end
